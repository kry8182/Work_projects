...
curs.execute("select max_update_dt from meta_all where table_name = 'tvorog_alkali_wash'")
date_from_meta = curs.fetchall()
if len(date_from_meta) > 0:
    t = date_from_meta[0][0]tvorog_alkali_wash
...
for i in range (0, len(df_tvg_alk_bak['Group1'])):
    if df_tvg_alk_bak['Group1'][i] > date_from_meta_str: 
        curs.execute(f"insert into tvorog_alkali_wash ( time, conc ) values (to_timestamp( '{df_tvg_alk_bak['Group1'][i]}', 'YYYY/MM/DD HH24:MI' ), '{df_tvg_alk_bak['WR00006'][i]}') ")
        c =+ c
