...
print('Reading files...')
path_tvg_alarm = ''
lstdir = os.listdir(r"C:/ETL/tvorog/Tvorog_journal")
for i in lstdir:
	if i.find('Alarm') > -1: 
		path_tvg_alarm = i
...
if path_tvg_alarm > '': print(path_tvg_alarm)
else: 
...
# # print( df_tvg_alarm.values.tolist())
# curs.executemany( "insert into tvorog_alkali_wash_stg( time, conc, number_wash ) values ( to_timestamp( %s, 'YYYY/MM/DD HH24:MI' ), %s, %s) ", df_tvg_alarm.values.tolist() )
...
backup('C:/ETL/tvorog/Tvorog_alkali_bak/' + path_tvg_alarm, 'arhive', '.backup')
