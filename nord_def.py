...
def ETL(table_name, path_source, path_destination):  # Находит csv-файлы в path_source, вставляет данные в таблицу table_name, cохраняет файлы в архив в path_destination 
  ...
  for file in files:
      ...
      if not os.path.exists(path_destination + folder):
        ...
        else: shutil.copytree(path_source + folder, path_destination + folder + '_ _')
...
sources_list = ['Analog_data_1s', 'Analog_data_1min', 'Analog_data_1h', 'alkali', 'alarm_data']
table_list = ['nord_1s', 'nord_1min', 'nord_1h', 'nord_alkali', 'nord_alarm']
k=0
