#!/usr/bin/python
# расчет значений и загрузка в таблицу steril_integrals_on_date данных за интервал времени
...
curs.execute("select max_update_dt from meta_all where table_name = 'steril_integrals_on_date'")
...
n = int(input('+ на сколько дней раньше еще расчитать: '))
temp_threshold = int(input('порог температуры: '))
again = True
while again:
    tt = (input('номер танка (формат 2 цифры): '))
    # if tt.startswith('0'): column_name = 't' + tt[-1] else:
    column_name = 't' + tt
    ...
    for i in range(0,n+1):
        interval = str(i)
        curs.execute(f''' insert into steril_integrals_on_date( date, {column_name},c{tt} ) 
        select 
            timestamp '{date}' - interval '{interval}' day,
            cast(avg(t) as numeric(5,2)),
            count(t)
            from (select 
            t_tank{tt} as t
           ...
           and table1.current_time between (timestamp '{date}' - interval '{interval}' day - interval '1' day)  and (timestamp '{date}' - interval '{interval}' day)
            ...
            update set 
                ...
                {column_name} = (select cast(avg(t) as numeric(5,2)) 
                from (select 
                t_tank{tt} as t
                from table1 where t_tank{tt} > {temp_threshold}
                and table1.current_time between (timestamp '{date}' - interval '{interval}' day - interval '1' day)  and (timestamp '{date}' - interval '{interval}' day)
    ...
    # Выводим результат
    curs.execute(f'''select cast(max({column_name}) as numeric(7,0)), cast(avg({column_name})as numeric(7,0)) from steril_integrals_on_date
    where  date between (timestamp '{date}' - interval '{interval}' day)  and (timestamp '{date}' ) ''')
    answer = curs.fetchall()
    print('Максимум =', answer[0][0])
    print('Cреднее =', answer[0][1])
    print('Макс/Cредн =', answer[0][0] / answer[0][1])
    while again and 1 == 1:
        yes_or_no = input('Еще танкки? (y/n):')
        if yes_or_no == 'y': break
        elif yes_or_no =='n': again = False


#  ОБНОВЛЕНИЕ МЕТАДАННЫХ 
print('Updating metadata...')
# sql_script('/home/de1m/krlv/sql_scripts/meta.sql', date_blk)
curs.execute("select max(date) from steril_integrals_on_date")
...
values ('steril_integrals_on_date', coalesce( to_timestamp( '{max_date_from_DWH_str}', 'YYYY/MM/DD HH24:MI:SS' ), to_date( '1900.01.01', 'YYYY.MM.DD' ))) 
