...
if path_tvg_alk_bak > '': print(path_tvg_alk_bak)
else: 
...
df_tvg_alk_bak = pandas.read_csv( 'C:/ETL/tvorog/Tvorog_alkali_bak/' + path_tvg_alk_bak, sep = ',', usecols=['Group1', 'WR00006'] )
# df_tvg_alk_bak
...
curs.execute("select max_update_dt from meta_all where table_name = 'tvorog_alkali_bak'")
...
curs.executemany( "insert into tvorog_alkali_wash_stg( time, conc) values ( to_timestamp( %s, 'YYYY/MM/DD HH24:MI' ), %s) ", df_tvg_alk_bak.values.tolist() )
...
# c = 0
# for i in range (0, len(df_tvg_alk_bak['Group1'])):
#     if df_tvg_alk_bak['Group1'][i] > date_from_meta_str: 
#         curs.execute(f"insert into tvorog_alkali_bak ( time, conc ) values (to_timestamp( '{df_tvg_alk_bak['Group1'][i]}', 'YYYY/MM/DD HH24:MI' ), '{df_tvg_alk_bak['WR00006'][i]}') ")
#         c =+ c
# print(c,' rows inserted')
# sql_script('/home/de1m/krlv/sql_scripts/transform_load.sql', date_term)
curs.execute(""" insert into tvorog_alkali_bak ( time, conc ) 
select
    stg.time,
    stg.conc
from  tvorog_alkali_wash_stg stg
left join tvorog_alkali_bak tgt
...
curs.execute("select max(time) from tvorog_alkali_bak")
...
values ('tvorog_alkali_bak', coalesce( to_timestamp( '{max_date_from_DWH_str}', 'YYYY/MM/DD HH24:MI:SS' ), to_date( '1900.01.01', 'YYYY.MM.DD' ))) 
...
backup('C:/ETL/tvorog/' + path_tvg_alk_bak, 'arhive', '.backup')
